...
Without it the site falls back to `https://www.focusrealm.org`, which is the
intended production domain, so a normal production build needs nothing set.

It is baked in **at build time**, not read at runtime — this is a static
export. If the site is ever served from another origin, rebuild with that value
or every canonical and Open Graph image URL will point at the wrong host:

```bash
NEXT_PUBLIC_SITE_URL=https://focusrealm-hotels.web.app npm run build
```

Any host matching `*.web.app`, `*.firebaseapp.com`, `*.vercel.app` or
`*.netlify.app` is automatically served `noindex`, so a preview link never
competes with the production domain.
...
mail client with a structured body. There is no backend: this is a static
export, so there is no server route to post to.

`components/forms/LeadForm.tsx` still contains the full `POST /api/lead` path,
gated behind `NEXT_PUBLIC_LEAD_API`. It is off by default. To wire a real inbox
you need a server runtime (Firebase App Hosting, or a function on another
host): restore a `/api/lead` route handler, drop `output: "export"` from
`next.config.ts`, and build with `NEXT_PUBLIC_LEAD_API=1`.

Until then the mail handoff is the honest path — nothing is accepted and then
quietly dropped.
...
closed to new Next.js projects, so this ships as plain files on the CDN;
...
Project is `focus-realm-1` and the Hosting site is **`fr-main-landing`**
(https://fr-main-landing.web.app) — set in `.firebaserc` and as `hosting.site`
in `firebase.json`. That project holds several sites, so the `site` key is what
keeps a deploy off `focus-realm-1`, `fr-hotels` and the rest.

`firebase.json` maps `out/` to the site with `cleanUrls`, sets the security and
caching headers, and — this part matters — states `Content-Type: image/png` for
`/icon`, `/apple-icon`, `/opengraph-image` and `/team/*/opengraph-image`. Next's
metadata routes export those without a file extension, so Firebase cannot infer
a type, and with `nosniff` set an unlabelled image will not render in a social
card.

To preview the exact artefact before deploying:

```bash
firebase emulators:start --only hosting   # http://127.0.0.1:5000
```

Note the emulator serves the files but does **not** apply the `headers` block
from `firebase.json` — headers can only be confirmed against a real deploy.

### Custom domain

The build hardcodes `https://www.focusrealm.org` in every canonical, Open Graph
URL and JSON-LD `@id`. Add that domain to the Firebase Hosting site and point
DNS at it; until the cutover completes those URLs resolve to whatever is
currently answering on that hostname.
